...
import { i as c, n as e, s as d } from './query-assigned-elements.js';
import { c as t } from './carbon-element.js';
import { C as s } from './tabs-extended.js';
import './content-item-row-media.js';
import './content-section-heading.js';
...
function a() {
  ...
  var c = {
    ...
    initializeInstanceElements: function (c, e) {
      ['method', 'field'].forEach(function (d) {
        e.forEach(function (e) {
          e.kind === d &&
            'own' === e.placement &&
            this.defineClassElement(c, e);
        }, this);
      }, this);
    },
    initializeClassElements: function (c, e) {
      var d = c.prototype;
      ['method', 'field'].forEach(function (t) {
        e.forEach(function (e) {
          var s = e.placement;
          if (e.kind === t && ('static' === s || 'prototype' === s)) {
            var o = 'static' === s ? c : d;
    ...
    defineClassElement: function (c, e) {
      var d = e.descriptor;
      if ('field' === e.kind) {
        var t = e.initializer;
        d = {
          ...
          value: void 0 === t ? void 0 : t.call(c),
        };
      }
      Object.defineProperty(c, e.key, d);
    },
    decorateClass: function (c, e) {
      var d = [],
        t = [],
        s = { static: [], prototype: [], own: [] };
      if (
        (c.forEach(function (c) {
          this.addElementPlacement(c, s);
        }, this),
        c.forEach(function (c) {
          if (!n(c)) return d.push(c);
          var e = this.decorateElement(c, s);
          d.push(e.element),
            d.push.apply(d, e.extras),
            t.push.apply(t, e.finishers);
        }, this),
        !e)
      )
        return { elements: d, finishers: t };
      var o = this.decorateConstructor(d, e);
      return t.push.apply(t, o.finishers), (o.finishers = t), o;
    },
    addElementPlacement: function (c, e, d) {
      var t = e[c.placement];
      if (!d && -1 !== t.indexOf(c.key))
        throw new TypeError('Duplicated element (' + c.key + ')');
      t.push(c.key);
    },
    decorateElement: function (c, e) {
      for (
        var d = [], t = [], s = c.decorators, o = s.length - 1;
        o >= 0;
        o--
      ) {
        var a = e[c.placement];
        a.splice(a.indexOf(c.key), 1);
        var i = this.fromElementDescriptor(c),
          r = this.toElementFinisherExtras((0, s[o])(i) || i);
        (c = r.element),
          this.addElementPlacement(c, e),
          r.finisher && t.push(r.finisher);
        var n = r.extras;
        if (n) {
          for (var l = 0; l < n.length; l++) this.addElementPlacement(n[l], e);
          d.push.apply(d, n);
        }
      }
      return { element: c, finishers: t, extras: d };
    },
    decorateConstructor: function (c, e) {
      for (var d = [], t = e.length - 1; t >= 0; t--) {
        var s = this.fromClassDescriptor(c),
          o = this.toClassDescriptor((0, e[t])(s) || s);
        if (
          (void 0 !== o.finisher && d.push(o.finisher), void 0 !== o.elements)
        ) {
          c = o.elements;
          for (var a = 0; a < c.length - 1; a++)
            for (var i = a + 1; i < c.length; i++)
              if (c[a].key === c[i].key && c[a].placement === c[i].placement)
                throw new TypeError('Duplicated element (' + c[a].key + ')');
        }
      }
      return { elements: c, finishers: d };
    },
    fromElementDescriptor: function (c) {
      var e = {
        kind: c.kind,
        key: c.key,
      ...
      return (
        ...
        'field' === c.kind && (e.initializer = c.initializer),
        e
      );
    },
    toElementDescriptors: function (c) {
      if (void 0 !== c)
        return ((e = c),
        (function (c) {
          ...
          (function (c, e) {
            if (c) {
              if ('string' == typeof c) return h(c, e);
              var d = {}.toString.call(c).slice(8, -1);
              return (
                'Object' === d && c.constructor && (d = c.constructor.name),
                'Map' === d || 'Set' === d
                  ? Array.from(c)
                  : 'Arguments' === d ||
          ...
          var e = this.toElementDescriptor(c);
          return (
            this.disallowProperty(c, 'finisher', 'An element descriptor'),
            this.disallowProperty(c, 'extras', 'An element descriptor'),
            e
          );
        }, this);
      var e;
    },
    toElementDescriptor: function (c) {
      var e = c.kind + '';
      if ('method' !== e && 'field' !== e)
        throw new TypeError(
      ...
      var s = c.descriptor;
      this.disallowProperty(c, 'elements', 'An element descriptor');
      var o = {
        kind: e,
        key: d,
        placement: t,
      ...
      return (
        'field' !== e
          ? this.disallowProperty(c, 'initializer', 'A method descriptor')
          : (this.disallowProperty(
            ...
            (o.initializer = c.initializer)),
        o
      );
    },
    toElementFinisherExtras: function (c) {
      return {
        element: this.toElementDescriptor(c),
        finisher: p(c, 'finisher'),
        extras: this.toElementDescriptors(c.extras),
      };
    },
    fromClassDescriptor: function (c) {
      var e = {
        kind: 'class',
        elements: c.map(this.fromElementDescriptor, this),
      };
      return (
        Object.defineProperty(e, Symbol.toStringTag, {
          value: 'Descriptor',
    ...
    toClassDescriptor: function (c) {
      ...
      this.disallowProperty(c, 'key', 'A class descriptor'),
        this.disallowProperty(c, 'placement', 'A class descriptor'),
        this.disallowProperty(c, 'descriptor', 'A class descriptor'),
        this.disallowProperty(c, 'initializer', 'A class descriptor'),
        this.disallowProperty(c, 'extras', 'A class descriptor');
      var d = p(c, 'finisher');
      return { elements: this.toElementDescriptors(c.elements), finisher: d };
    },
    runClassFinishers: function (c, e) {
    ...
    disallowProperty: function (c, e, d) {
      if (void 0 !== c[e])
        throw new TypeError(d + " can't have a ." + e + ' property.');
    },
  };
  return c;
}
function i(c) {
  var e,
    d = b(c.key);
  'method' === c.kind
    ? (e = { value: c.value, writable: !0, configurable: !0, enumerable: !1 })
    : 'get' === c.kind
    ? (e = { get: c.value, configurable: !0, enumerable: !1 })
    : 'set' === c.kind
    ? (e = { set: c.value, configurable: !0, enumerable: !1 })
    : 'field' === c.kind &&
      (e = { configurable: !0, writable: !0, enumerable: !0 });
  var t = {
    kind: 'field' === c.kind ? 'field' : 'method',
    key: d,
    placement: c.static ? 'static' : 'field' === c.kind ? 'own' : 'prototype',
    descriptor: e,
  };
  return (
    c.decorators && (t.decorators = c.decorators),
    'field' === c.kind && (t.initializer = c.value),
    t
  );
}
function r(c, e) {
  void 0 !== c.descriptor.get
    ? (e.descriptor.get = c.descriptor.get)
    : (e.descriptor.set = c.descriptor.set);
}
function n(c) {
  return c.decorators && c.decorators.length;
}
function l(c) {
  return void 0 !== c && !(void 0 === c.value && void 0 === c.writable);
}
function p(c, e) {
  var d = c[e];
...
function h(c, e) {
  ...
  for (var d = 0, t = Array(e); d < e; d++) t[d] = c[d];
  return t;
}
const { stablePrefix: g } = d;
!(function (c, e, d, t) {
  var s = a();
  if (t) for (var o = 0; o < t.length; o++) s = t[o](s);
  var p = e(function (c) {
      s.initializeInstanceElements(c, b.elements);
    }, d),
    b = s.decorateClass(
      (function (c) {
        for (
          ...
          var s,
            o = c[t];
          if ('method' === o.kind && (s = e.find(d)))
            if (l(o.descriptor) || l(s.descriptor)) {
              if (n(o) || n(s))
  ...
  s.initializeClassElements(p.F, b.elements),
    s.runClassFinishers(p.F, b.finishers);
})(
  [t(`${g}-tabs-extended-media`)],
  function (c, d) {
    return {
      F: class extends d {
        constructor(...e) {
      ...
      d: [
        {
          kind: 'field',
          decorators: [e({ attribute: 'section-heading', reflect: !0 })],
          key: 'sectionHeading',
          value: () => 'true',
        },
        {
  ...
  console.warn(
    'The tabs-extended-media component has been deprecated in favor of tabs-extended component. See tabs-extended documentation for more information.'
  );
